...
import { Sparkles, LayoutDashboard, Star, LogOut, ChevronDown, Menu, X, Sun, Moon, BarChart2, Bell } from 'lucide-react';
...
export default function Navbar() {
  ...
  // Busca badge de cards vencidos a cada 5 min
  useEffect(() => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      <nav className={`mx-4 mt-4 glass rounded-2xl border px-6 py-3 flex items-center justify-between ${isDark ? 'border-white/8' : 'border-black/8'}`}>
        {/* Logo */}
        ...
        <div className="flex items-center gap-2">
          ...
          <div ref={dropdownRef} className="relative hidden md:block">
            <button onClick={() => setDropdownOpen((v) => !v)}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}>
              ...
              <span className={`text-sm font-medium ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{user?.name?.split(' ')[0]}</span>
              <ChevronDown size={14} className={`transition-transform text-slate-500 ${dropdownOpen ? 'rotate-180' : ''}`} />
            </button>
            {dropdownOpen && (
              <div className={`absolute right-0 top-full mt-2 w-48 glass rounded-xl border overflow-hidden shadow-xl ${isDark ? 'border-white/10' : 'border-black/8'}`}>
          ...
          {/* Mobile hamburger */}
          <button className={`md:hidden transition-colors ${isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}
      ...
      {/* Menu mobile */}
      {mobileOpen && (
        <div className={`md:hidden mx-4 mt-1 glass rounded-2xl border p-4 flex flex-col gap-1 ${isDark ? 'border-white/8' : 'border-black/8'}`}>
          {navLinks.map(({ to, label, icon: Icon }) => (
            <Link key={to} to={to} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm transition-colors ${isDark ? 'text-slate-300 hover:bg-white/5 hover:text-white' : 'text-slate-600 hover:bg-black/5 hover:text-slate-900'}`}>
              <Icon size={16} /> {label}
            </Link>
          ))}
          <div className={`border-t mt-2 pt-2 ${isDark ? 'border-white/5' : 'border-black/5'}`}>
            <button onClick={toggle}
              className={`w-full flex items-center gap-2 px-4 py-3 rounded-xl text-sm transition-colors ${isDark ? 'text-yellow-300 hover:bg-yellow-400/10' : 'text-blue-500 hover:bg-blue-500/10'}`}>
